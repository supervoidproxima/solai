...
| `test_compiled.py` | 43 | 7 on loading a vault's own declarations, 7 on the fate of a hand edit, 11 on the upgrade merge, 4 on a vault with no manifest, 3 on the folder a kept class lives in, 4 on a class both sides declare, 7 on an artefact whose format holds no stamp |

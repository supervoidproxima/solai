...
```
python tests/run_tests.py                        # 362 assertions
...
| `test_compiled.py` | 32 | 7 on loading a vault's own declarations, 7 on the fate of a hand edit, 11 on the upgrade merge, 4 on a vault with no manifest, 3 on the folder a kept class lives in |
...
so 361 of 362 passing is red for the same reason a failure is. Adding an assertion means
raising `EXPECTED`
...
class lives in, which is deliberately a name no archetype declares. `predates` removes the compiled

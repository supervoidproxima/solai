...
```
python tests/run_tests.py                        # 366 assertions
...
| `test_compiled.py` | 36 | 7 on loading a vault's own declarations, 7 on the fate of a hand edit, 11 on the upgrade merge, 4 on a vault with no manifest, 3 on the folder a kept class lives in, 4 on a class both sides declare |
...
so 365 of 366 passing is red for the same reason a failure is. Adding an assertion means
raising `EXPECTED`

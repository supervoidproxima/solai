...
| **R-6** | `scaffold.py "<vault>" --harvest [--all]`: every copied file a vault has changed since the package wrote it, diffed against what the package ships now, and `release.py` naming the vaults not read since an older version | The package's only feedback path from a running vault ran through somebody's memory, and it failed twice in two days. Deferred since v0.1.0 and buildable only after 0.34.0, which is when "this vault changed a file we ship" became a fact the engine holds rather than a guess every copied file read `LOCAL` about | `done` 2026-09-13, at solai 0.37.0. Decided in the Solai vault's `CHG-020`, built the same day. Its first real run found the Counselor's signed `stamp_check.py` carrying a `.js` and `.html` page-stamp check the package has never had, which is one more instance of exactly what it was built to stop losing. What it does not solve stands as written: somebody still runs it, and a package cannot discover a vault it was never told about |
...
| 0.36.0 | a whole file a vault keeps is signed for, the way a region already could | 69 |
| 0.35.0 | what a wikilink is, spelled once, and it allows the escaped pipe a table cell requires | 68 |

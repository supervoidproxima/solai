...
| **R-6** | `/solai harvest <vault>`: read every copied file a vault has changed since the package wrote it, print the diff against what the package ships now, and have `release.py` name the vaults not yet harvested | Decided 2026-09-13, after the second instance in two days. `GAP-013` lived correct in a vault for days while the package shipped the defect to everyone including that vault, and reached the package only because an upgrade overwrote the file and a count moved; `check_binaries.py` dropping a check it used to carry is the same failure again. The shape is buildable only now: before 0.34.0 every copied file read `LOCAL`, so the signal did not exist, which is what made deferring it right and what has now expired. A checklist is what exists and is what failed, twice. A release step that reaches into vaults is refused by the package's own rule and needs a registry of vault paths whose staleness nothing detects, which is the argument that killed the sha ancestry in `RES-012`. What it does not solve, named: somebody still runs it, and a package cannot discover a vault it was never told about. It reduces the dependency from remembering what you improved to remembering one command, and the release prints that | `open`, next |
| **R-16** | Decide what a card over the chunk cap does: split the way a clause does, or keep the truncation and say so at the record | The first dry run over the Solai vault cut 17 of 24 card records at the 1,800-character default. A clause over the cap splits into numbered parts, because a passage that looks whole and has lost the sentence carrying its condition is worse than an obviously partial one; a card over the cap is taken as `text[:cap]` and only the `truncated` flag records it. Both cannot be right, and the honest answer may be neither: a card may be the one unit whose head does carry its claim. `GAP-006` in the Solai vault holds the evidence: 17 of 24 records cut in the first dry run over it | `open`, a decision rather than a task |
...
| **R-10** | Merge artefacts, fragments and runtime on `--from-package` | 0.36.0 took the accounting half: a file a vault keeps is signed for under a change record and says when the package has moved since, so the divergence is named rather than anonymous. The MERGE itself is what stays deferred. The stated reason was "a vault that changed one has a hand edit", and `GAP-012` falsified it with nine counterexamples in one vault, none of them edited. 0.34.0 took the half that was owed: a copied file records what it was copied from, so the package changing it is a different fact from the vault editing it. The MERGE half stays deferred, and now for a reason that holds: a vault that really has edited a copied script keeps it, and reconciling that with a new version needs a three-way merge against an ancestor no vault carries |
